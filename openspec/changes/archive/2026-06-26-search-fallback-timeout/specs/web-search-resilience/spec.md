...
O sistema SHALL abortar a chamada ao `gpt-4o-search-preview` se ela não concluir dentro de **45 segundos**. O timeout SHALL ser implementado via `AbortSignal` passado ao SDK OpenAI, cancelando efetivamente a requisição HTTP.

#### Scenario: Timeout acionado na primeira tentativa
- **WHEN** a chamada ao `gpt-4o-search-preview` demora mais de 45 segundos sem resposta
